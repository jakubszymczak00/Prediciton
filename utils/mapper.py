def get_generation_by_year(marka, model, rok):
    """
    Zwraca generację (lub kod modelu) na podstawie marki, modelu i rocznika.
    Baza obejmuje:
    - Wszystkie popularne dostawczaki.
    - Pełną gamę osobowych (Audi A1-A8, Q; BMW 1-8, X; Mercedes A-S, GL; VW, Ford, itd.).
    
    Zasada: Kolejność lat MALEJĄCA (od najnowszych).
    """
    ...
    baza = {
        ...
        # --- MERCEDES --- (Scalony z dostawczymi, jeśli już istnieją w słowniku)
    }
    
    # Dodatkowe słowniki do scalenia
    mercedes_osobowe = {
    ...
    if "MERCEDES-BENZ" in baza:
        baza["MERCEDES-BENZ"].update(mercedes_osobowe)
    else:
        baza["MERCEDES-BENZ"] = mercedes_osobowe


    # =========================================================
    #                  SAMOCHODY OSOBOWE (POPULARNE)
    # =========================================================
    
    baza_popularne = {
        "VOLKSWAGEN": { # Dopisujemy do istniejącego klucza
        ...
        "RENAULT": { # Dopisujemy do istniejącego
        ...
        "PEUGEOT": { # Dopisujemy do istniejącego
        ...
        "CITROEN": { # Dopisujemy do istniejącego
    ...
    # Scalanie słownika 'baza_popularne' do 'baza'
    for mk, modele in baza_popularne.items():
        if mk in baza:
            baza[mk].update(modele)
        else:
            baza[mk] = modele

    if marka in baza:
        sorted_models = sorted(baza[marka].keys(), key=len, reverse=True)
        for model_key in sorted_models:
            if model_key in model:
                ranges = baza[marka][model_key]
